...
class StepService extends Service {
  ...
  async InserOrUpdate(stepId, payload) {
    const { ctx, service } = this;
    const instance = await this.model.find(stepId);
    if (!instance) {
      // 未关注新增
      return this.model.create(payload);
    }
    return this.model.update(stepId, payload);
  }
  // destroy======================================================================================================>
  async destroy(_id) {
    const { ctx, service } = this;
    const instance = await service.step.find(_id);
    if (!instance) {
