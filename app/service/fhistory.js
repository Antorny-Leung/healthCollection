...
class FhistoryService extends Service {

  get model() {
    return this.ctx.model.Fhistory;
  }
  ...
  async destroy(_id) {
    const { ctx, service } = this;
    const instance = await service.fhistory.find(_id);
    if (!instance) {
  ...
  async show(_id) {
    ...
    const instance = await service.fhistory.find(_id);
    if (!instance) {
      ctx.throw(404, 'fhistory not found');
    }
...
module.exports = FhistoryService;
