...
class OxyhemoglobinService extends Service {
  ...
  async destroy(_id) {
    const { ctx, service } = this;
    const instance = await service.oxyhemoglobin.find(_id);
    if (!instance) {
  ...
  async show(_id) {
    ...
    const instance = await service.oxyhemoglobin.find(_id);
    if (!instance) {
      ctx.throw(404, 'oxyhemoglobin not found');
    }
...
module.exports = OxyhemoglobinService;
