...
class BloodglucoseService extends Service {
  ...
  async destroy(_id) {
    const { ctx, service } = this;
    const instance = await service.bloodglucose.find(_id);
    if (!instance) {
  ...
  async show(_id) {
    ...
    const instance = await service.bloodglucose.find(_id);
    if (!instance) {
      ctx.throw(404, 'bloodglucose not found');
    }
...
module.exports = BloodglucoseService;
